...
class PHEHXClass():
    ...
    def OutputList(self):
        ...
        return [
            ...
            ('Outlet Superheat','K',self.Tin_c-self.Tdew_c or self.Tin_c),
            ('Q Total','W',self.Q),
    ...
    def DetermineHTBounds(self):
        ...
        logging.debug("In determine HT bounds")
        ...
        #TODO: added by Suzy for testing on coaxial
        if self.AS_h.has_melting_line():
            tempMeltHot = self.AS_h.melting_line(CP.iT, CP.iP, self.pin_h)
        else:
            tempMeltHot = 0
        temperatureHot = tempMeltHot if self.Tin_c < tempMeltHot else self.Tin_c
        
        
        self.AS_h.update(CP.PT_INPUTS, self.pin_h, temperatureHot)
...
#            raise ValueError('Outlet or inlet of PHE is pinching.  Why?')
    ...
    def HTDP(self,AS,T,p,mdot,side=None):
        """
        This function calls mainly the heat transfer and Fde drop
        ...
        if self.HXType == 'Plate-HX':
            ...
            logging.debug("diameterHydraulic: %g", Outputs['Dh'])
    ...
    def BuildEnthalpyLists(self,Q):
        logging.debug("In BuildEnthalpyLists")
        ...
        eps = 1e-3
        logging.debug("enthalpyListsHot: %s", EnthalpyList_h)
        logging.debug("enthalpySatVaporHot: %g", hsatV_h)
        logging.debug("enthalpySatLiquidHot: %g", hsatL_h)
        logging.debug("enthalpyListsCold: %s", EnthalpyList_c)
        logging.debug("enthalpySatVaporCold: %g", hsatV_c)
        logging.debug("enthalpySatLiquidCold: %g", hsatL_c)
        if hsatV_c<EnthalpyList_c[-1]-eps and hsatV_c>EnthalpyList_c[0]+eps:
            logging.debug("enthalpySatVaporCold between first and last enthalpies")
            EnthalpyList_c.insert(len(EnthalpyList_c)-1,hsatV_c)
        if hsatL_c<EnthalpyList_c[-1]-eps and hsatL_c>EnthalpyList_c[0]+eps:
            logging.debug("enthalpySatLiquidCold between first and last enthalpies")
            EnthalpyList_c.insert(1,hsatL_c)

        if hsatV_h<EnthalpyList_h[-1]-eps and hsatV_h>EnthalpyList_h[0]+eps:
            logging.debug("enthalpySatVaporHot between first and last enthalpies")
            EnthalpyList_h.insert(len(EnthalpyList_h)-1,hsatV_h)
        if hsatL_h<EnthalpyList_h[-1]-eps and hsatL_h>EnthalpyList_h[0]+eps:
            logging.debug("enthalpySatLiquidHot between first and last enthalpies")
            EnthalpyList_h.insert(1,hsatL_h)
        
        logging.debug("enthalpy lists hot: %s", EnthalpyList_h)
        logging.debug("enthalpy lists cold: %s", EnthalpyList_c)
        I_h=0
        I_c=0
        while I_h<len(EnthalpyList_h)-1:
            ...
            I_h+=1
            I_c+=1
        logging.debug("enthalpy list hot: %s", EnthalpyList_h)
        logging.debug("enthalpy list cold: %s", EnthalpyList_c)
...
#             self.h_2phase_c=0
    ...
    def eNTU_CounterFlow(self,Cr,Ntu):
        ...
        """
        # logging.debug("In eNTU_CounterFlow")
    ...
    def _OnePhaseH_OnePhaseC_Qimposed(self,Inputs):
        ...
        logging.debug("In _OnePhaseH_OnePhaseC_Qimposed")
        ...
        logging.debug("heat transfer coeff hot: %g", h_h)
        logging.debug("heat transfer coeff cold: %g", h_c)
        logging.debug("conductance hot: %g", 1/(h_h*self.A_h_wetted))
        logging.debug("conductance cold: %g", 1/(h_c*self.A_c_wetted))
        logging.debug("total conductance: %g", UA_total)
        logging.debug("capacitance: %s", C)
        logging.debug("capacitanceMin: %g", Cmin)
        logging.debug("capacitanceRatio: %g", Cr)
        logging.debug("qMax: %g", Qmax)
        logging.debug("epsilon: %g", epsilon)
        logging.debug("ntu: %g", NTU)
        logging.debug("conductanceRequired: %g", UA_req)
        logging.debug("thermalFraction: %g", w)
        logging.debug("densityHot: %g", rho_h)
        logging.debug("chargeHot: %g", Charge_h)
        logging.debug("densityCold: %g", rho_c)
        logging.debug("chargeCold: %g", Charge_c)
    ...
    def _OnePhaseH_TwoPhaseC_Qimposed(self,Inputs):
        ...
        logging.debug("In _OnePhaseH_TwoPhaseC_Qimposed")
        ...
        while abs(change)>1e-6:
            ...
            logging.debug("areaWettedHot: %g", self.A_h_wetted)
            logging.debug("areaWettedCold: %g", self.A_c_wetted)
            logging.debug("heatTransferCoeffSinglePhase: %g", h_h)
            logging.debug("heatTransferCoeffTwoPhase: %g", h_c_2phase)
            logging.debug("thermalResistanceWall: %g", self.Rw)
            logging.debug("conductanceTotal: %g", UA_total)
            logging.debug("capacitanceSinglePhase: %g", C_h)
            logging.debug("tempDiff: %g", (Inputs['Tin_h']-Inputs['Tsat_c']))
            logging.debug("qMax: %g", Qmax)
            logging.debug("epsilon: %g", epsilon)
            logging.debug("ntu: %g", NTU)
            logging.debug("conductanceRequired: %g", UA_req)
            logging.debug("fractionUpdated: %g", UA_req/UA_total)
            logging.debug("thermalFraction: %g", w)
    ...
    def _TwoPhaseH_OnePhaseC_Qimposed(self,Inputs):
        ...
        logging.debug("In _TwoPhaseH_OnePhaseC_Qimposed")
    ...
    def _TransCritPhaseH_TwoPhaseC_Qimposed(self,Inputs):
        ...
        logging.debug("In _TransCritPhaseH_TwoPhaseC_Qimposed")
        #Mole mass of refrigerant for Cooper correlation
        M=self.AS_c.molar_mass()*1000 #[kg/kmol]
        #Reduced pressure for Cooper Correlation
        pcrit_c = self.AS_c.p_critical() #critical pressure of Ref_c [Pa]
    ...
    def _TransCritPhaseH_OnePhaseC_Qimposed(self,Inputs):
        ...
        logging.debug("In _TransCritPhaseH_OnePhaseC_Qimposed")
    ...
    def Calculate(self):
        ...
        def GivenQ(Q):
            ...
            logging.debug("In GivenQ. Q is %g", Q)
            if Q == 0.0:
                return -1
            if Q == self.Qmax:
                return np.inf
            ...
            logging.debug("enthalpyListCold after BuildEnthalpyLists: %s", EnthalpyList_c)
            logging.debug("enthalpyListHot after BuildEnthalpyLists: %s", EnthalpyList_h)
...
#            pylab.show()
            ...
            while I_h<len(EnthalpyList_h)-1:
                ...
                logging.debug("tempInC: %g", Tin_c)
                logging.debug("tempOutC: %g", Tout_c)
                logging.debug("enthalpyInC: %g", hin_c)
                logging.debug("enthalpyOutC: %g", hout_c)
                logging.debug("pressure in cold: %g", self.pin_c)
                logging.debug("tempBubbleCold: %g", self.Tbubble_c or 0.0)
                logging.debug("tempDewCold: %g", self.Tdew_c or 0.0)
                logging.debug("densitySatLiquidCold: %g", self.rhosatL_c or 0.0)
                logging.debug("densitySatVaporCold: %g", self.rhosatV_c or 0.0)
                logging.debug("enthalpyInH: %g", hin_h)
                logging.debug("enthalpyOutH: %g", hout_h)
                logging.debug("tempInH: %g", Tin_h)
                logging.debug("tempOutH: %g", Tout_h)
                if Phase_h in ['Subcooled','Superheated'] and Phase_c in ['Subcooled','Superheated']:
                    ...
                    if self.Verbosity>6:
                        logging.debug('w[1-1]: %g', Outputs['w'])
                    ...
                    if self.Verbosity>6:
                        logging.debug('w[2-1]: %g', Outputs['w'])
                    wList.append(Outputs['w'])
                    cellList.append(Outputs)
                    ...
                    if self.Verbosity>6:
                        logging.debug('w[3-2]: %g', Outputs['w'])
                    wList.append(Outputs['w'])
                    cellList.append(Outputs)
                    ...
                    if self.Verbosity>6:
                        logging.debug('w[3-1]: %g', Outputs['w'])
                    wList.append(Outputs['w'])
                    cellList.append(Outputs)
                    ...
                    if self.Verbosity>6:
                        logging.debug('w[1-2]: %g', Outputs['w'])
                    wList.append(Outputs['w'])
                    cellList.append(Outputs)
            ...
            logging.debug("wlist: %s", wList)
            if self.Verbosity>6:
                logging.debug('wsum: %s', np.sum(wList))
        ...
        try:
            ...
            logging.debug(GivenQ(low), GivenQ(hi))
            raise
        # Collect parameters from all the pieces
        logging.debug("cellList: %s", self.cellList)
...
def SWEPVariedmdot():
    ...
    for mdot_h in [0.4176,0.5013,0.6267,0.8357,1.254,2.508]:
        ...
        PHE=PHEHXClass(**params)
        PHE.Calculate()
        logging.info(PHE.Q,',',PHE.h_subcooled_h,',',-PHE.DP_h/1000)
        logging.info(PHE.OutputList())
...
#        QQ.append(PHE.Q)
#        TT.append(Tin)
#        logging.info(PHE.Q, PHE.Q/PHE.Qmax)
#    pylab.plot(TT,QQ)
#    pylab.show()
...
#        logging.info(PHE.Q/PHE.Qmax,PHE.Q)
        logging.info("mdot at end of loop: %s", mdot)
        logging.info((PHE.Q,',',PHE.h_subcooled_h,',',-PHE.DP_h/1000))
        logging.info(PHE.OutputList())
    logging.info("mdot: %s", mdot)
    logging.info("QQ: %s", QQ)
    logging.info("Q1: %s", Q1)
    pylab.plot(mdot,QQ)
    pylab.show()

#TODO: added by Suzy to test coaxial heat exchanger
def CoaxialEHX():
    TT=[]
    QQ=[]
    Q1=[]
...
#        Tdew_evap=285.42
        ...
        params={
                'HXType':'Coaxial-HX', #choose the type of IHX
                ...
                'mdot_h':0.38,
                'hin_c':h,
                'pin_c':PropsSI('P','T',Tdew_evap,'Q',1.0,Ref_r),
                'pin_h':300000,     #pin_g in Pa
                'hin_h':PropsSI('H', 'T', 290.52,'P', 300000, Ref_g),
                'AS_c':AS_r, #Abstract state of refigerant
                'AS_h':AS_g, ##Abstract state of glycol
                'Verbosity':0,
                'Conductivity' : 237, #[W/m-K]
                }
        IHX=PHEHXClass(**params)
        IHX.Calculate()
        
        TT.append(Tdew_evap)
        QQ.append(IHX.h_2phase_c)#IHX.Q)
        Q1.append(IHX.h_superheated_c)
...
def BDW16DW():
    ...
    ref_c = "MEG"
    backend_r = "HEOS"
    backend_c = "IncompressibleBackend"
    AS_r = CP.AbstractState(backend_r, ref_r)
    AS_c = CP.AbstractState(backend_c, ref_c)
    ...
    volume_channel_single = 0.000061
    params = {
        'AS_c':AS_c,
        'mdot_c':mf_W_condenser,
        'pin_c':p_W_in,
        'hin_c':PropsSI('H','P',p_W_in,'T',T_W_supply_in,'INCOMP::MEG-50%'), #[J/kg-K]

        'AS_h':AS_r,
        'mdot_h':mf_R_cycle,
        'pin_h':p_r_in,
        'hin_h':PropsSI('H','T',T_R_cond_in,'P',p_r_in,ref_r), #[J/kg-K]

        #Geometric parameters
        'HXType':'Plate-HX', #choose the type of IHX
        'Bp' : 0.072,
        'Lp' : 0.329, #Center-to-center distance between ports
        'Nplates' : 12,
        ...
        'Rp': 1.0, #[microns] Surface roughness
        'MoreChannels' : 'Cold', #Which stream gets the extra channel, 'Hot' or 'Cold'
        'Verbosity':7,
    ...
    plateHEX = PHEHXClass(**params)
    plateHEX.Calculate()
    # print(plateHEX.OutputList())
    print(plateHEX.Tout_h)
    print(plateHEX.Tout_c)

if __name__=='__main__':
    logging.basicConfig(filename='PHEHX.log', encoding='utf-8', level=logging.DEBUG)
    BDW16DW()
    # CoaxialEHX()
    # SamplePHEHX()
    # WyattPHEHX()
