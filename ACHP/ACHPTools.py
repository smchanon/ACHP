...
def Write2CSV(Class,filename,append=False):
    """
    This function takes in a class and a filename pointer
    """
    def OL2Strings(OL):
        head=str(OL[0][0])
        units=str(OL[0][1])
    ...
    if True: #if isinstance(Class,(SecondaryCycleClass,DXCycleClass)):
        ...
        #find items with an outputlist and collect it
        for item in list(filter(lambda a: hasattr(a, 'OutputList'), dir(Class))):
    ...
    if os.path.exists(filename):
        ...
        firstRow=True
    appendLetter = 'a' if append else 'w'

    with open(filename, appendLetter) as fP:
        if append and not firstRow:
            fP.write(vals+'\n')
        else:
            if IsCycle:
                fP.write(components+'\n')
...
def simple_write_to_file(head,data,file,append='True',newline=True):
    #function to simply write some data comma separated into a file
    #quick and dirty, nonspecific, therefore no formatting...
